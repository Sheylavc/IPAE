//
//  PreguntasIViewController.swift
//  DiagnosticoIPAE
//
...
class PreguntasIViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        idTest = Test.getidTest(2)
        nSubCategorias = SubCategoria.getSubCategoriasByCategoria(2)?.count
    ...
    @IBAction func leftButton(sender: AnyObject) {
        let pagina = prefs.stringForKey("paginaInnova")
        if Int(pagina!) > 1{
            prefs.setValue(Int(pagina!)! - 1, forKey: "paginaInnova")
            getPregunta()
        }
    }
    ...
    func getPregunta(){
        ...
        if let subCateg = prefs.stringForKey("subcatInnova"){
        ...
        //categoria = Int(subCat.idCategoria!)
        ...
        if let pagina = prefs.stringForKey("paginaInnova"){
            pageNum = Int(pagina)
        }else{
            pageNum = 1
            prefs.setValue(pageNum , forKey: "paginaInnova")
        }
        ...
        if let pos = prefs.stringForKey("poscatInnova"){
            ...
            posAcelera = 0
            prefs.setValue(0 , forKey: "poscatInnova")
        }
    ...
    func setPagina(){
        if(pageNum == total_paginas){
            if (nSubCategorias - 1) == Int(prefs.stringForKey("poscatInnova")!){
                self.performSegueWithIdentifier("showFormInnova", sender: 2)
            }else{
                prefs.setValue(1, forKey: "paginaInnova")
                prefs.setValue(Int(subCategoria)! + 1, forKey: "subcatInnova")
                prefs.setValue(posAcelera + 1, forKey: "poscatInnova")
                getPregunta()
            }
    ...
    func createButtons(){
        ...
        buttonNO.addTarget(self, action: #selector(PreguntasViewController.actionNo(_:)), forControlEvents: UIControlEvents.TouchUpInside)
    ...
    func put3Buttons(){
        ...
        imgCircle.image = UIImage(named:"circle_i")!
    ...
    func put4Buttons(){
        ...
        imgCircle.image = UIImage(named:"circle2_i")!
    ...
    {
        if segue.identifier == "showFormInnova"
        {
